...
2. **Chiave AI**: procurati una chiave **Claude** (`sk-ant-…`) **oppure Gemini** (`AIza…`). Ne basta una. Si inserisce nella schermata iniziale del gioco (campo "Assistente AI") e resta **solo nel browser** del PC. Va inserita su **ogni** PC dei gruppi.
   - Senza chiave: il gioco funziona, ma **il testimone della Fase 3 non risponde** e non compaiono i commenti del consulente. Con chiave tutto attivo.
...
| **Fase 1 — Triage** | ~20 min | Primi **8 min**: solo analisi dati **con Claude**, slider bloccati. Poi simulano (max 3 tentativi) |
| **Fase 2 — Rilancio** | ~16 min | Lock **5 min**; a **~7 min** lo **chef si licenzia** (pop-up): devono rivedere il personale |
| **Fase 3 — Indagine** | ~10 min | Parte il cronometro: ogni secondo e ogni accusa sbagliata costano. Trovano il colpevole |
| **Speech / pitch** | ~22 min | I gruppi migliori presentano; la giuria valuta (offline) |
| Buffer | ~10 min | Margine + premiazione |

**Dove cambiare i tempi** (nel file `restaurant_game.html`, in alto nello `<script>`):
- `COOLDOWN_MS = { 1: 8*60*1000, 2: 5*60*1000 }` → minuti di studio bloccato per Fase 1 e Fase 2.
- `CHEF_QUIT_MS = 7*60*1000` → quando si licenzia lo chef in Fase 2.

Se sei lungo: salta il tutorial per-gruppo (resta solo la tua demo) o accorcia la Fase 3.
...
| Quando | Password |
|---|---|
| Far entrare in **Fase 2 — Rilancio** | `Un!credit01` |
| Far entrare in **Fase 3 — Il sabotatore** | `D4t4p1zz4` |
...
- **Punteggio**: è un saldo cumulato. Il risultato della Fase 1 alimenta il budget della Fase 2; nella Fase 3 il tempo e le accuse sbagliate **sottraggono** dal saldo.
